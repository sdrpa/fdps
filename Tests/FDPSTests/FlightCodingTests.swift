...
class FlightCodingTests: XCTestCase {
    ...
    func testCoding() {
        ...
        let value = Flight(callsign: "ASL123", squawk: 2000, position: position, mach: 0.745, heading: 180, flightPlan: flightPlan)
        guard let encoded = value.toJSON() else {
            XCTFail(); return
        }
        XCTAssertTrue(JSONSerialization.isValidJSONObject(encoded))
        let decoded = Flight(json: encoded)
        XCTAssertEqual(value, decoded)
    }

    static var allTests : [(String, (FlightCodingTests) -> () throws -> Void)] {
        return [
